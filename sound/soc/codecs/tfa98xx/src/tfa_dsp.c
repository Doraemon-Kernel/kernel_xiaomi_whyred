...
int tfa_reset(struct tfa_device *tfa)
{
	...
	if (tfa->verbose) {
		if (((tfa->tfa_family == 1) && state != TFA_STATE_RESET) ||
			...
			pr_debug(
				"WARNING: Device reset should be performed in POWERDOWN state\n");
		}
	}
...
int tfa_dev_set_tdm_bitwidth(struct tfa_device *tfa, int width)
{
	...
	case 24: /* 24-bit sample in 32-bit slot */
		nbck = 2;
		slotlen = 31;
		samplesize = (tfa->tfa_family == 1) ?
				     23 :
				     (tfa->is_probus_device == 1) ? 31 : 23;
		break;
...
int tfa_plop_noise_interrupt(struct tfa_device *tfa, int profile, int vstep)
{
	...
	if (tfa_irq_get(tfa, tfa9912_irq_stnoclk)) {
		...
		if (no_clk == 1) {
			...
			pr_debug(
				"No clock detected. Resetting the I2CR to avoid pop on 72! \n");
			...
			if (err != tfa_error_ok) {
				...
				pr_debug(
					"Setting i2c registers after I2CR succesfull\n");
			...
			*/
			if (strstr(tfaContProfileName(tfa->cnt, tfa->dev_idx,
				   ...
				   ".saam")) {
				pr_debug(
					"Powering down from a SAAM profile, workaround PLMA4766 used! \n");
